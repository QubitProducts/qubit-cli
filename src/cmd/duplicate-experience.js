...
module.exports = async function duplicateExperience (propertyId, experienceId, destinationPropertyId) {
  try {
    const pkg = await getPkg()
    if (!experienceId) experienceId = _.get(pkg, 'meta.experienceId')
    if (!propertyId) propertyId = _.get(pkg, 'meta.propertyId')
    if (!propertyId) propertyId = await suggest.property(formatLog('What property would you like to duplicate from?'))
    if (!propertyId) return
    if (!experienceId) experienceId = await suggest.experience(propertyId)
    ...
    const duplicateOptions = { id: experienceId, name: `${experience.name} (copy)`, target_property_id: destinationPropertyId }
